...
public class viewall extends JFrame
{
	...
	public viewall(String title)
	{
		...
		try
		{
			...
			rs = stmt.executeQuery("SELECT Appointment.AppID, Appointment.vName, Appointment.vMobile, Appointment.Purpose,  Contact.cName, Appointment.vLocation, Appointment.vDate, Appointment.vTime, Appointment.Duration FROM Contact INNER JOIN Appointment ON Contact.contatID = Appointment.ContactID");
			int row = 0;
			while (rs.next())
			{
