	...
	//		contact = (String)txtWhom.getSelectedItem();
			...
			if(flag)
			{
				try
				{
					int contact=0;
					con = conn.getConnection();
					ps = con.prepareStatement("Select contactID From Contact Where cName =?");
					ps.setString(1,whom);
					ResultSet rs = ps.executeQuery();
					if(rs.next())
					{
						...
						try
						{
								...
								ps = con.prepareStatement("SELECT Appointment.AppID, Appointment.vName, Appointment.vMobile, Appointment.Purpose,  Contact.cName, Appointment.vLocation, Appointment.vDate, Appointment.vTime, Appointment.Duration FROM Contact INNER JOIN Appointment ON Contact.contactID = Appointment.contactID WHERE vDate Like ? AND contactID = ?",ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
